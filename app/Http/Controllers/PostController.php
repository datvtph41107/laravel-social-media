...
class PostController extends Controller
{
    ...
    // tôi hiểu theo khía cạnh này
    // do function update có 3 phương thức trên của cơ sở dữ liệu nên cần dùng DB:: để cập nhật tính nhất quán
    ...
    public function update(UpdatePostRequest $request, Post $post)
    {
        ...
        try {
            ...
            $post->update($data);
            // 1, 2, 3, 4
            $deleted_id = $data['deleted_file_id'] ?? [];
    ...
    public function destroy(Post $post)
    {
        ...
        return response("You dont have permision to delete this post", 403);
    }

    public function dowloadAttachment(PostAttachment $attachment)
    {
    ...
    public function createComment(Request $request, Post $post)
    {
        $data = $request->validate([
            'comment' => ['required'],
            // khi sub comment reply parent id se ton tai cua id comment goc
            'parent_id' => ['nullable', 'exists:comments,id']
        ]);
        $comment = Comment::create([
            'post_id' => $post->id,
            'comment' => nl2br(htmlentities($data['comment'])),
            'user_id' => Auth::id(),
