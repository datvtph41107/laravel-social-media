...
class GroupController extends Controller
{
    ...
    {
        $group->load('currentUserGroupJoin');
        ...
        return Inertia::render('Group/View', [
            'success' => session('success'),
            // new object lay theo 1 doi tuobg
            'group' => new GroupResource($group),
            // new UserResoure($users)
            // collection laay theo mang
            'posts' => $posts,
            'users' => GroupUserResource::collection($users),
            'requests' => UserResoure::collection($requests),
        ]);
    }
    ...
    {
        // {{group-slug}}
        // select * from `groups` where `slug` = laravel-developer-web limit 1
    ...
    public function changeRole(Request $request, Group $group)
    {
        ...
        $data = $request->validate([
            'user_id' => ['required'],
            'valueRole' => ['required', Rule::enum(GroupUserRole::class)]
        ]);
        ...
        if ($groupUser) {
            $groupUser->role = $data['valueRole'];
            $groupUser->save();

            $groupUser->user->notify(new RoleChanged($group, $data['valueRole']));
